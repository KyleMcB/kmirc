...
data class MODE(
    ...
    constructor(ircMessage: IIrcMessage) : this(from = IrcFrom.fromPrefix(
        ircMessage.prefix ?: throw IllegalIRCMessage(
            "MODE message missing prefix", ircMessage
        )
        ...
        if (modes == null) throw IllegalIRCMessage(
            "modes message missing second parameter. The list of mode changes", ircMessage
        )
        ...
        if (minusIndex == -1) {
            ...
            val end: Int = modes.indexOf('+', startIndex = minusIndex + 1).takeUnless { it == -1 } ?: modes.lastIndex
